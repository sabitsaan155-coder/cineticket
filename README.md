...
- [ ] Отдельная прикладная роль `manager` подготовлена через Django Groups / Permissions, но ещё не выведена в отдельный пользовательский сценарий
...
- Довести роль `manager` до полностью демонстрируемого сценария в интерфейсе и документации.
